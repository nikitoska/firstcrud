...
<div>
    ...
    <form:form action="${searchuser}" commandName="searchuser">
        <form:label path="name"><spring:message text="Search user by name:"/></form:label>
...
<ul class="pagination">
    <c:if test="${page > 1}">
        <li><a href="<c:url value="/"/>">First</a></li>
        <li><a href="<c:url value="/"><c:param name="page" value="${page - 1}"/>${page - 1}</c:url>">Prev</a></li>
    </c:if>
    <li><spring:message text="${page}"/></li>
    <c:if test="${!empty listUsers}">
        <li><a href="<c:url value="/"><c:param name="page" value="${page + 1}"/>${page + 1}</c:url>">Next</a></li>
    </c:if>
</ul>
</body>
</html>
